...
AUTHOR_CHANNEL = "@project_eliminator"
...
{get_text('features')}
• 📥 Скачивать видео и фото до 50МБ
• 🖼️ Сохранять изображения и фото
• ⚡ Быстрая обработка ссылок
• 📊 Красивый прогресс-бар
{'• 👑 Премиум: до 100МБ и быстрые обновления' if is_premium else ''}
{'• 🔧 Админ: безлимитные скачивания и админ-панель' if is_admin_user else ''}
...
{'• 👑 Premium: up to 100MB and fast updates' if is_premium else ''}
{'• 🔧 Admin: unlimited downloads and admin panel' if is_admin_user else ''}
...
🔍 Поиск видео{' 👑' if is_premium else ''}...
    ...
    try:
        ...
        if file_path and os.path.exists(file_path):
            ...
            # Проверяем лимит размера (только премиум и админ могут скачивать больше 50MB)
            max_size = 50 * 1024 * 1024  # Базовый лимит 50MB для всех
            
            # Расширенные лимиты для премиум и админа
            if is_admin(message.from_user.id):
...
Видео слишком большое ({size_mb:.1f}МБ > {size_info})

Попробуйте другую ссылку или активируйте премиум для скачивания больших файлов.
                    """.strip()
